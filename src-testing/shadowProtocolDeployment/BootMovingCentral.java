 ...
 * Deployment testing.
 */
public class BootMovingCentral {
	/**
	 * Performs test
	 * 
	 * @param args_
	...
	public static void main(String[] args_) {
		...
		args += " -package testing src-testing.shadowProtocolDeployment.Scripts test.simplePingPong -loader agent:mobileComposite ";
		
		for(int i = 0; i < 4; i++) {
			String srv = i % 2 == 0 ? server[i / 2] : server[(i - 1) / 2];
			args += " -node node" + i + "-" + srv + " -monitor time: -pylon webSocket:Pylon" + i;
			args += (i % 2 == 0 ? " serverPort:8885" : " connectTo:ws://localhost:8885");
			// if(i % 2 == 0)
