...
public class AgentTest extends Unit implements Agent
{
	 ...
	 */
	protected static final String	SHARD_ENDPOINT				= "ml_description";

	/**
	 * Endpoint element for other shards.
	 */
	protected static final String   OTHER_SHARD_ENDPOINT        = "ml_description";
	...
	public boolean start()
	{
		...
		if(otherAgents != null) {
			for(String otherAgent : otherAgents) {
				lf("Sending the messages to ", otherAgent);

				if(!msgShard.sendMessage(AgentWave.makePath(getName(), SHARD_ENDPOINT),
						AgentWave.makePath(otherAgent, OTHER_SHARD_ENDPOINT), "{\"type\": \"ml_ask\", \"task\": \"DigitClassification\""))
					le("Message sending failed");

				if(!msgShard.sendMessage(AgentWave.makePath(getName(), SHARD_ENDPOINT),
					AgentWave.makePath(otherAgent, OTHER_SHARD_ENDPOINT), "{\"type\": \"ml_task\", \"task\": \"DigitClassification\""))
					le("Message sending failed");

				if(!msgShard.sendMessage(AgentWave.makePath(getName(), SHARD_ENDPOINT),
					AgentWave.makePath(otherAgent, OTHER_SHARD_ENDPOINT), "{\"type\": \"ml_transfer\", \"task\": \"DigitClassification\""))
					le("Message sending failed");
			}
		}
