...
public class PrimeNumberCalculatorShardForComposite extends AgentShardCore {
    ...
    public void signalAgentEvent(AgentEvent event)
    {
        if(event.containsKey(AbstractMessagingShard.SOURCE_PARAMETER)){
            if(event.get(AbstractMessagingShard.SOURCE_PARAMETER) == "Master" ) {
                primeNumbersLimit = Integer.parseInt(
                        event.getValue(AbstractMessagingShard.CONTENT_PARAMETER));
                isWaiting = false;
                agentName = AbstractMessagingShard.DESTINATION_PARAMETER;
            }
        }
    ...
    private void printMessage(AgentEvent event) {
        System.out.println("SLAVE: " + event.get(AbstractMessagingShard.CONTENT_PARAMETER) + " de la "
                + AbstractMessagingShard.SOURCE_PARAMETER + " la " +
