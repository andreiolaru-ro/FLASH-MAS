...
public enum CategoryName {
	 ...
	 * is specified.
	 */
	NODE(new CatPar().isIdentifiable().canBeAutoGenerated().hasParent(DEPLOYMENT, Is.MANDATORY)),
	/**
	 ...
	 */
	AGENT(new CatPar().isIdentifiable().hasParent(SUPPORT, Is.OPTIONAL, CatPar.CAN_ADD_AUTO)),
	/**
	 * Shards to be deployed in agents (hierarchical key).
	 */
	SHARD(new CatPar().hasParent(AGENT, Is.MANDATORY).isPortableFrom(DEPLOYMENT)),
	 ...
	 */
	DEPLOYMENT_FILE(new CatPar().isValue().isUnique().hasParent(NODE, Is.OPTIONAL).isPortableFrom(DEPLOYMENT)),
	/**
	 * Java packages that contain classes needed in the deployment (simple key, all values are relevant).
	 */
	PACKAGE(new CatPar().isValue().hasParent(AGENT, Is.OPTIONAL).isPortableFrom(DEPLOYMENT)),
	/**
	 ...
	 */
	LOAD_ORDER(new CatPar().isValue().isUnique().hasParent(NODE, Is.OPTIONAL).isPortableFrom(DEPLOYMENT)),
	...
	private static class CatPar
	{
		...
		{
			return hasParent(_parent, parent_optional, false);
		}
		 ...
		 * @return the CatPar instance.
		 */
		CatPar hasParent(CategoryName _parent, Is parent_optional, boolean autoadd)
		{
			parent = _parent;
