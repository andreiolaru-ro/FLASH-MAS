...
/**
 * The messaging component should handle all communication between the agent and
 * other agents. Note that the existence of such a component in the
 * {@link CompositeAgent} modifies its behavior: without any messaging
 * component, received messages are notified as agent events to all components
 * of the agent; with it, messages are passed to the messaging component, which
 * then should route them to the appropriate receivers.
 * 
 * <h2>Endpoints</h2> The communication is abstracted internally to the agent as
 * exchanging messages between endpoints, where each endpoint is identified by a
 * {@link String} address composed of multiple elements separated by slashes (by
 * default {@link #ADDRESS_SEPARATOR}={@value #ADDRESS_SEPARATOR}). This means
 * that an endpoint may be identified by an URI (if the
 * {@link MessagingShard} implementation supports it), or, for instance, by
 * an address of the type "Agent1/Visualization" (this will work with Jade,
 * where agents are addressable by name).
 * <p>
 * Messages are sent from one endpoint to another, and contain a {@link String}.
 * Specific implementations may parse the string for additional structure.
 * <p>
 * We make a difference between [complete] endpoints (or paths) and internal
 * endpoints (or paths).
 * <p>
 * <b>IMPORTANT NOTE:</b> An agent address (returned by
 * {@link #getAgentAddress}) concatenated with an internal path should result in
 * a complete path. Internal paths should begin with a slash. Otherwise, the
 * predefined methods for parsing and assembling endpoint paths may not work
 * correctly.
 * 
 * <h2>Using messaging features</h2> In principle, other components should not
 * need to call the [implementation of the] MessagingComponent explicitly.
 * Functionality of the messaging component should be used by means of the
 * protected methods in {@link AgentComponent}, such as
 * <code>registerMessageReceiver</code>, <code>sendMessage</code> and
 * <code>getComponentEndpoint</code>. Messages will be signaled, as they are
 * processed from the agent's event queue, by means of the specified receivers.
 * 
 * <h2>Extending this class</h2> The abstract class implements some basic
 * methods for working with slash-delimited addresses, and offers the method for
 * registering message handlers. The class also eases the task of posting agent
 * events to the agent thread, by means of the method {@link #receiveMessage}
 * that should be called by extending classes.
 * <p>
 * Basically, a class extending {@link MessagingShard} should implement the
 * following:
 * <ul>
 * <li>if the messaging system uses agent names as their addresses, then it is
 * recommended to extend the {@link NameBasedMessagingComponent} class.
 * <li>if it needs to access the platform containing the agent and implementing
 * communication, it should do that after {@link #atAgentStart} is called (at
 * any point before this method is called the agent may not be loaded onto the
 * platform yet). To get a reference to the platform, by means of the
 * {@link #getPlatformLink} method.
 * <li>it should implement the {@link #sendMessage(String, String, String)}
 * method that sends a message to another agent.
 * <li>when a message is received, the implementation should call the
 * {@link #receiveMessage} of the parent, that will pack the information into an
 * {@link AgentEvent} instance and post it in the event queue. After the event
 * is picked from the event queue, {@link MessagingShard} will use the
 * <code>handleMessage()</code> method to distribute the message to receiving
 * components.
 * </ul>
 * 
 * 
 * @author Andrei Olaru
 */
public abstract class MessagingShard extends AgentShardCore {
	/**
	...
	/**
	 * The {@link Map} of {@link AgentEventHandler} instances that were registered
	 * with this component, associated with their respective endpoints. Multiple
	 * handlers may be registered with the same endpoint. These handlers will be
	 * invoked in no particular order. The endpoint is an internal path, rather than
	 * a complete path.
	 */
	 ...
	 * Default constructor.
	 */
	public MessagingShard() {
		super(AgentShardDesignation.standardFeature(StandardAgentShard.MESSAGING));
	}

	/**
	 * Extending classes should call this method to defer to
	 * {@link MessagingShard} the effort of packing message data into an
	...
	protected void receiveMessage(String source, String destination, String content) {
		...
		try {
			event.add(MessagingShard.SOURCE_PARAMETER, source);
			event.add(MessagingShard.DESTINATION_PARAMETER, destination);
			event.add(MessagingShard.CONTENT_PARAMETER, content);
...
//		return getAgentAddress(getAgent().getAgentName());
	...
	/**
	 * The method produces the string address of an agent on the same platform,
	 * being provided with the agent name.
	 * <p>
	 * This address can subsequently be suffixed with more path elements by using
	 * {@link #makePath(String, String...)}.
	 * <p>
