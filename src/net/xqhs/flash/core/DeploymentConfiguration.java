 ...
 * {@link MultiTreeMap}), in which the first level are entity types, and other each type entities are listed by name
 * (if any).
 * 
 * @author Andrei Olaru
 */
public class DeploymentConfiguration extends MultiTreeMap
{
			...
			ContentHolder<XMLTree> loadedXML) throws ConfigLockedException
	{
		...
		MultiTreeMap allNodes = this.getTree(CategoryName.NODE.getName());
		for(String catName : this.getKeys())
		{
			...
			for(String node : allNodes.getHierarchicalNames())
	...
	protected void readXML(XMLNode node, MultiTreeMap _nodeTree, MultiTreeMap rootTree, UnitComponentExt log)
	{
		...
		for(XMLNode child : node.getNodes())
		{
			...
			else
			{
				...
				MultiTreeMap childTree = nodeTree.getTree(catName, true).getTree(name, true);
			...
			if(isCategory(a))
			{
				...
				while(!context.isEmpty())
				{
					...
					{ // category is known and found the correct parent for the category
						MultiTreeMap c = context.peek().elemTree.getTree(catName, true);
				...
				if(context.isEmpty())
				{
					...
					else
					{
						...
						{ // category should be inside the root category but is not a root-category-parented portable
							...
							MultiTreeMap implicitCat = rootTree.getTree(ROOT_CATEGORY.getName(), true);
							MultiTreeMap implicitElem = implicitCat.getTree(null, true);
