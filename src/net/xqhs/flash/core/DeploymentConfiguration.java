...
public class DeploymentConfiguration extends MultiTreeMap
{
			...
			ContentHolder<XMLTree> loadedXML) throws ConfigLockedException
	{
		...
		// if no node is present (e.g. the deployment contains no agents), create a node to serve as the node for the
		// current machine.
		if(!deployment.isHierarchical(CategoryName.NODE.s()) && CategoryName.NODE.canBeAutoGenerated())
		{
			log.li("Creating an empty node.");
			integrateName(new MultiTreeMap(), CategoryName.NODE,
					integrateChildCat(deployment, CategoryName.NODE.s(), log), this, log);
			// post process again
			postProcess(deployment, CategoryName.DEPLOYMENT.s(), new MultiTreeMap(), new MultiTreeMap(),
					new LinkedList<String>(), this, log);
		}
