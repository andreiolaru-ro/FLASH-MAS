...
public class MonitoringShard extends AgentShardGeneral {
	 ...
	 * @param parent the parent element to remove the element from (if null, the root element is used)
	 * @return true if the element was removed, false otherwise
	 */
	public boolean removeGuiElement(Element element, Element parent){
		if (parent == null) parent = this.interfaceSpecification;
		...
		for (Element e : parent.getChildren()){
			if (e.equals(element)){
				...
				if (parent==this.interfaceSpecification) { sendGuiUpdate(new Yaml().dump(this.interfaceSpecification)); }
				le("Element [] removed from interface specification", element.toString());
				return true;
			}
			else if (e.getChildren() != null){
				if (removeGuiElement(element, e)) return true;
			}
		}
	 ...
	 * @param interfaceSpecification the interface specification to send
	 */
