...
/**
 * Class to create messages and monitoring notifications.
 * 
...
public class OldMessageFactory {
	...
	public enum MessageType {
		...
		REQ_LEAVE,
		/**
		 * Message sent from Region-Server to the Region-Server from the birthplace of the agent, when that agent wants
		 * to leave to another node. Contains the next fields: type, node (pylon name), source (sender server name),
		 * agentName (the agent name that wants to leave)
		 */
		REQ_BUFFER,
		/**
		 * Message sent from Region-Server to the Region-Server from the birthplace of the agent, when the agent arrives
		 * in the new region. Contains the next fields: type, node (pylon name), source (agent name), lastLocation
		 * (agent last location)
		 */
		AGENT_UPDATE,
		/**
	...
	/**
	 * Types of actions for monitoring notifications.
	 * 
	 * @author Monica Pricope
	 */
	 ...
	 * @param content
	 *            - information extra
	 * @return - returns an object with type String with the complete message
	 */
	 ...
	 *            - information extra
	 * @param time
	 *            - timestamp for this notification.
	 * @return - returns an object with type String
	 */
