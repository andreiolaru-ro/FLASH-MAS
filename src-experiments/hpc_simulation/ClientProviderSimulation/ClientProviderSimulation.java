...
enum ProviderServices {
	...
	EVEN_NUMBERS, ODD_NUMBERS, FIBONACCI, NUMBER_MULTIPLES, QUADRATIC_EQUATIONS
...
public class ClientProviderSimulation {
	...
	public static void addContextToUserAgentsList(LocalPylon pylon, ArrayList<UserAgent> agentList) {
	...
	public static void addContextToProviderAgentsList(LocalPylon pylon, ArrayList<ProviderAgent> agentList) {
	...
	public static void addShardsToUsersList(ArrayList<UserAgent> users) {
		...
		for(int i = 0; i < users.size(); i++) {
			users.get(i).addMessagingShard(new LocalPylon.SimpleLocalMessaging());
			users.get(i).addUserRequestShard(new UserRequestShard(
					AgentShardDesignation.customShard(UserRequestShard.USER_REQUEST_SHARD_DESIGNATION)));
		}
	}
	...
	public static void addShardsToProvidersList(ArrayList<ProviderAgent> providers) {
		...
		for(int i = 0; i < providers.size(); i += chunk) {
			for(int j = i; j < i + chunk; j++) {
				...
				/* Add messaging shard */
				agent.addShard(new LocalPylon.SimpleLocalMessaging());
	...
	public static AgentShardCore getShardFromService(ProviderServices serice) {
		...
		switch(serice) {
		case EVEN_NUMBERS:
			return new EvenNumbersShard(
					AgentShardDesignation.customShard(EvenNumbersShard.EVEN_NUMBERS_SHARD_DESIGNATION));
		
		case ODD_NUMBERS:
			return new OddNumbersShard(
					AgentShardDesignation.customShard(OddNumbersShard.ODD_NUMBERS_SHARD_DESIGNATION));
		...
		case NUMBER_MULTIPLES:
			return new NumberMultiplesCountShard(
					AgentShardDesignation.customShard(NumberMultiplesCountShard.NUMBER_MULTIPLES_SHARD_DESIGNATION));
		
		case QUADRATIC_EQUATIONS:
			return new QuadraticEquationSolverShard(AgentShardDesignation
					.customShard(QuadraticEquationSolverShard.QUADRATIC_EQUATION_SHARD_DESIGNATION));
	...
	public static void main(String[] args) {
		...
		/*
		 * Pot face mai multe scenarii: 1. pentru fiecare user, exista un provider liber care il poate ajuta; 2. exista
		 * mai putini provideri decat useri
		 */
		
		/* Create supervisor agent */
		SupervisorAgent supervisorAgent = new SupervisorAgent("Supervisor");
		supervisorAgent.addContext(pylon.asContext());
		supervisorAgent.addMessagingShard(new LocalPylon.SimpleLocalMessaging());
